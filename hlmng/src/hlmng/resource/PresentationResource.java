...
public class PresentationResource  {
	...
	@GET
	@Path("{id}")
	public Presentation getPresentation(@PathParam("id") String id,
			@Context HttpHeaders headers,
			@Context HttpServletResponse servletResponse) throws IOException{
		Object obj =  GenDaoLoader.instance.getPresentationDao().getElement(id);
		ResourceHelper.sendErrorIfNull(obj,response);
		Presentation evt=(Presentation) obj;
		return evt;	
	}
	...
	public Response putPresentation(Presentation element,@PathParam("id") String id) {
		Response res;
		if (GenDaoLoader.instance.getPresentationDao().getElement(id)!=null){
			GenDaoLoader.instance.getPresentationDao().updateElement(element, id);
