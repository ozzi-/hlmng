...
/**
 * This class dynamically builds SQL Prepared Queries (so with = ? etc.).
...
public class QueryBuilder {
	...
	/**
	 * Builds a map for every "FK" (foreign key) containing field. Map<FKName,Query>
	 * @param className
	 */
	public static Map<String,String> BuildJoinQuery(String className) {
		System.out.println("Building Join Query for "+className);
		...
		try {
			...
			for (Field field : methods) {
				if(field.getName().contains("FK")){
					map.put(field.getName(),"SELECT * FROM "+tableName+" WHERE "+field.getName()+" = ?;");
				}
			}
	...
	/**
	 * Creates something like >( fieldName1, fieldName2, ...)< or if boolean set is true >fieldname1=?, fieldName2=?, ...<
	 * @param methods
	...
	/**
	 * Creates something like >values ( ? , ? , ? )< , assuming there are 3 methods in the array param
	 * @param methods
	 * @return
	 */
	 ...
	 *  // listByID =  select * from table where fieldNameFK = ?
	 */
	public enum opType{
